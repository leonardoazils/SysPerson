...
{
    public static class CaracterizacaoCapitalExtension
    {
        public static IEnumerable<SelectListItem> DropDownList(this IEnumerable<CaracterizacaoCapital> listaEntidades)
        {
            return listaEntidades.OrderBy(x => x.Descricao).Select(x => new SelectListItem { Text = x.Descricao, Value = x.Id.ToString() });
        }

        public static IEnumerable<SelectListItem> DropDownList(this IEnumerable<CaracterizacaoCapital> listaEntidades, Guid idSelecionado)
        {
