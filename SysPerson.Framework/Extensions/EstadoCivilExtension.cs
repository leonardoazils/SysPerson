...
{
    public static class EstadoCivilExtension
    {
        public static IEnumerable<SelectListItem> DropDownList(this IEnumerable<EstadoCivil> listaEntidades)
        {
            return listaEntidades.OrderBy(x => x.Descricao).Select(x => new SelectListItem { Text = x.Descricao, Value = x.Id.ToString() });
        }

        public static IEnumerable<SelectListItem> DropDownList(this IEnumerable<EstadoCivil> listaEntidades, Guid idSelecionado)
        {
